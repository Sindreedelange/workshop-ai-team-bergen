...
for (const task of ["strukturer-dokument", "strukturer-dokument-fallback", "les-dokumentside"]) {
  assert.equal(reasoningForOppgave(task), false);
  assert(Object.hasOwn(OPPGAVE_REASONING, task), `${task} skal ha en eksplisitt policy`);
}
...
assert.doesNotMatch(PROVIDER_REASONING.ollama.grunn, /qwen|mistral|standardmodellen/i,
  "Ollama-begrunnelsen skal beskrive integrasjonen, ikke anta hvilken modell som er valgt");
assert.match(PROVIDER_REASONING.ollama.grunn, /valgte modellen/);
assert.match(kilde, /const providerReasoning = PROVIDER_REASONING\[aiProvider\]/,
  "Helse skal bruke den samme providerbegrunnelsen");
assert.match(kilde, /grunn: providerReasoning\?\.grunn/,
  "Providerbegrunnelsen skal følge helsesvaret");
