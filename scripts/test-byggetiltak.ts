...
import type { GarasjeGrunnlag, GarasjeTiltak, GarasjeVurdering } from "../apps/shared/garasje.ts";
import { evaluateGarasje, getByggetiltakPlanvarsler, validateByggetiltak, validateGarasjeTiltak } from "../apps/sandbox-backend/src/garasje.ts";
import { buildGarasjeTiltak, normalizeGarasjeSvar } from "../apps/sandbox-backend/src/garasje-prosess.ts";
...
const grunnlag: GarasjeGrunnlag = {
...
const check = (result: GarasjeVurdering, id: string) => {
...
assert.deepEqual(validateGarasjeTiltak(legacy), legacy);
const { bebygdEiendom: _legacyBebyggelse, ...legacyWithoutBebyggelse } = legacy;
assert.throws(() => validateGarasjeTiltak(legacyWithoutBebyggelse), badRequest);
...
for (const fixture of [fixtures.frittliggende, fixtures.tilbygg]) {
  ...
  assert.equal(evaluateGarasje(request, grunnlag).utfall, "maa_avklares");
  assert.throws(() => validateByggetiltak({ ...request, bebygdEiendom: "ja" }), badRequest);
}
assert.equal(evaluateGarasje(legacy, grunnlag).tiltakstype, "frittliggende");
assert.equal(evaluateGarasje(legacy, grunnlag).nasjonaltUnntak, "oppfylt");
assert.equal(evaluateGarasje({ ...legacy, bya: 50.01 }, grunnlag).utfall, "soknadspliktig");
assert.equal(evaluateGarasje({ ...legacy, avstandNabogrense: null }, grunnlag).utfall, "maa_avklares");
assert.match(check(evaluateGarasje(legacy, grunnlag), "kommuneplan").forklaring, /fradelt.*bebygd.*LNF-verdiene/);
assert.match(check(evaluateGarasje(legacy, grunnlag), "kommuneplan").forklaring, /1 m.*ikke tilstrekkelig/);
...
for (const fixture of Object.values(fixtures)) {
  ...
  const normalized = normalizeGarasjeSvar({ ...common, ...fixture });
  assert.equal(normalized.tiltakstypeBekreftet, "ja");
  assert.equal(normalized.bebygdEiendom, undefined);
  const reconstructed = buildGarasjeTiltak(normalized);
  assert.equal("tiltakstype" in reconstructed && reconstructed.tiltakstype, fixture.tiltakstype);
  assert.equal(evaluateGarasje(reconstructed, grunnlag).utfall, "maa_avklares");
  assert.throws(() => normalizeGarasjeSvar({ ...common, ...fixture, tiltakstypeBekreftet: null }), badRequest);
  for (const field of getByggetiltakSporsmaal(fixture.tiltakstype)) {
    ...
    assert.throws(() => normalizeGarasjeSvar({ ...common, ...missing }), badRequest);
    if (fixture.tiltakstype !== "frittliggende") {
      const unknown = { ...fixture, [field.id]: null };
      assert.equal(evaluateGarasje(validateByggetiltak(unknown), grunnlag).utfall, "maa_avklares");
      assert.equal(normalizeGarasjeSvar({ ...common, ...unknown })[field.id], "vet-ikke");
    }
  }
}
...
assert.throws(() => normalizeGarasjeSvar({ ...common, ...fixtures.gjerde, bra: 0 }), badRequest);
...
assert.equal(normalizeGarasjeSvar({ ...common, ...fixtures.gjerde, hoyde: "0,9" }).hoyde, 0.9);

for (const change of [{ bra: 15.01 }, { bya: 15.01 }, { etasjer: 3 }, { understottet: false }, { nyBoenhet: true }, { endrerBruk: true }]) {
  assert.equal(evaluateGarasje({ ...fixtures.tilbygg, ...change }, grunnlag).utfall, "soknadspliktig");
}
assert.equal(check(evaluateGarasje(fixtures.tilbygg, grunnlag), "tilbygg-areal").status, "oppfylt");
assert.equal(check(evaluateGarasje({ ...fixtures.tilbygg, bra: 0 }, grunnlag), "tilbygg-areal").status, "oppfylt");
assert.match(check(evaluateGarasje(fixtures.tilbygg, grunnlag), "tilbygg-nabogrense").forklaring, /1-metersregel gjelder ikke/);
assert.equal(check(evaluateGarasje(fixtures.gjerde, grunnlag), "gjerde-hoyde").status, "oppfylt");
assert.equal(evaluateGarasje({ ...fixtures.gjerde, hoyde: 1.5 }, grunnlag).nasjonaltUnntak, "oppfylt");
assert.equal(evaluateGarasje({ ...fixtures.gjerde, hoyde: 1.51 }, grunnlag).utfall, "soknadspliktig");
assert.equal(evaluateGarasje({ ...fixtures.gjerde, friSikt: false }, grunnlag).utfall, "soknadspliktig");
assert.equal(evaluateGarasje({ ...fixtures.gjerde, motVeg: false, hoyde: 1.6 }, grunnlag).nasjonaltUnntak, "oppfylt");
for (const hoyde of [0.9, 0.91]) {
  const assessment = evaluateGarasje({ ...fixtures.gjerde, hoyde }, grunnlag);
...
for (const changed of [
  ...
  assert(!evaluateGarasje(fixtures.gjerde, changed).sjekker.some(item => item.id === "gjerde-plan-6170063"));
  assert(!getByggetiltakPlanvarsler("gjerde", changed).some(item => item.id === "gjerde-plan-6170063"));
}
assert(!evaluateGarasje(legacy, grunnlag).sjekker.some(item => item.id === "gjerde-plan-6170063"));
assert.equal(check(evaluateGarasje(fixtures.fasade, grunnlag), "fasade-karakter").status, "oppfylt");
assert.equal(check(evaluateGarasje({ ...fixtures.fasade, likUtforming: false, endrerUtseende: true }, grunnlag), "fasade-karakter").status, "uavklart");
assert.equal(evaluateGarasje({ ...fixtures.fasade, endrerBaering: true }, grunnlag).utfall, "soknadspliktig");
assert.equal(evaluateGarasje({ ...fixtures.fasade, endrerBrannkrav: true }, grunnlag).utfall, "soknadspliktig");
assert(evaluateGarasje(fixtures.fasade, grunnlag).nesteSteg?.some(step => /bilder.*fagperson/.test(step)));
assert.equal(check(evaluateGarasje(fixtures.ukjent, grunnlag), "tiltakstype").status, "uavklart");
...
try {
  ...
  const early = await runRessurs(state, "GET", new URL(`http://localhost/api/garasje/grunnlag?${earlyQuery}`),
    { sporingsId: "test-byggetiltak", kaller: caller }) as GarasjeGrunnlag;
  ...
  assert.equal(evaluateGarasje(fixtures.gjerde, early).utfall, "maa_avklares");
  assert.deepEqual(check(evaluateGarasje(fixtures.gjerde, early), earlyFence.id), earlyFence);
  ...
  for (const fixture of Object.values(fixtures)) {
    ...
    const direct = await runRessurs(state, "GET", new URL(`http://localhost/api/garasje/sjekk?${query}`),
      { sporingsId: "test-byggetiltak", kaller: caller }) as { vurdering: GarasjeVurdering };
    ...
    const embedded = await runRessurs(state, "GET", new URL(`http://localhost/api/garasje/prosess/sjekk?personId=${person.personId}`),
      { sporingsId: "test-byggetiltak", kaller: caller, personId: person.personId, oekt: session, steg: definition.steg[3] }) as { vurdering: GarasjeVurdering };
