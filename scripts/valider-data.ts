...
// must not.
const krr = await read("data/krr.json");
const KRR_ALDERSGULV = 15;
const KRR_SPRAAK = new Set(["nb", "nn", "en"]);

const skalHaKrr = new Set(
  personer
    .filter((p) => p.personstatus === "BOSATT" &&
      (alderVed(String(p.foedselsdato), satser.gjelderFra) ?? 0) >= KRR_ALDERSGULV)
    .map((p) => p.syntetiskFodselsnummer)
);
const harKrr = new Set(krr.map((r: any) => r.fnr));
if (harKrr.size !== krr.length) {
  throw new Error("data/krr.json har to rader med samme fnr.");
}
for (const rad of krr) {
  if (!identer.has(rad.fnr)) {
    throw new Error(`KRR-rad peker på ukjent fnr ${rad.fnr}.`);
  }
  if (!skalHaKrr.has(rad.fnr)) {
    throw new Error(
      `KRR-rad for ${rad.fnr}, som ikke er bosatt og ${KRR_ALDERSGULV} år eller mer. ` +
      `Registeret skal bare dekke dem.`
    );
  }
  if (!KRR_SPRAAK.has(rad.spraak)) {
    throw new Error(
  ...
  if (rad.kanVarsles !== forventetKanVarsles) {
    throw new Error(
      `KRR-rad for ${rad.fnr}: kanVarsles=${rad.kanVarsles}, men reservert=${rad.reservert} ` +
      `og kontaktinfo ${rad.epost || rad.tlf ? "finnes" : "mangler"} gir ${forventetKanVarsles}.`
    );
  }
}
for (const fnr of skalHaKrr) {
  if (!harKrr.has(fnr)) {
    throw new Error(
      `${fnr} er bosatt og ${KRR_ALDERSGULV} år eller mer, men mangler i data/krr.json. ` +
      `Kjør node scripts/importer-tenor.ts.`
    );
  }
}
...
// back on.
const krrPerFnr = new Map(krr.map((r: any) => [r.fnr, r]));
let kuratertReservertMedAdresse = 0;
for (const kilde of kuratert.personer) {
  if (!kilde.krr) continue;
  const rad: any = krrPerFnr.get(kilde.syntetiskFodselsnummer);
  if (!rad) {
    throw new Error(
      `${kilde.personId} har forfattet krr i data/kuratert.json men ingen rad i data/krr.json.`
    );
  }
  for (const felt of ["reservert", "spraak"] as const) {
  ...
  const adresse = kilde.bostedsadresse || {};
  if (kilde.krr.reservert && adresse.adressenavn && adresse.postnummer && adresse.poststed) {
    kuratertReservertMedAdresse += 1;
  }
}
