...
import type { TiltakshjelpenAdresse, TiltakshjelpenGrunnlag, TiltakshjelpenVurdering } from "../apps/shared/tiltakshjelpen.ts";
...
type Result = { melding: string; grunnlag: TiltakshjelpenGrunnlag; vurdering: TiltakshjelpenVurdering; sporingsId: string };
...
const { buildTiltakshjelpenSok, buildTiltakshjelpenTiltak, normalizeTiltakshjelpenSvar } = await import("../apps/sandbox-backend/src/tiltakshjelpen-prosess.ts");
...
try {
  ...
  const revisjon = JSON.parse(await readFile(path.join(stateDir, "revisjonslogg.json"), "utf8")) as {
    handling: string; ressurs: string; sporingsId: string; grunnlag?: { datagrunnlag?: TiltakshjelpenGrunnlag };
  }[];
  ...
  console.log("Tiltakshjelpens prosess: begge eiere, svarvalidering, hjemmel, nye oppslag, retry, ukjent, revisjon og fullføring uten søknad besto uten eksternt nettverk.");
} finally {
