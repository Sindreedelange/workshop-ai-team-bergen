...
assert.equal(buildClientKonfigurasjon({ TILTAKSHJELPEN_BACKEND_PUBLIC_URL: "http://localhost:19080" }).backendBaseUrl, config.backendBaseUrl);
assert.equal(buildClientKonfigurasjon({ TILTAKSHJELPEN_IDPORTEN_PUBLIC_URL: "http://localhost:19086" }).idportenBaseUrl, config.idportenBaseUrl);
assert.equal(buildClientKonfigurasjon({
  TILTAKSHJELPEN_BACKEND_PUBLIC_URL: "http://localhost:19080", GARASJE_BACKEND_PUBLIC_URL: "http://localhost:18080"
}).backendBaseUrl, config.backendBaseUrl);
assert.equal(buildClientKonfigurasjon({
  BACKEND_PUBLIC_URL: "http://localhost:19080", TILTAKSHJELPEN_BACKEND_PUBLIC_URL: "http://localhost:18080"
}).backendBaseUrl, config.backendBaseUrl);
...
for (const page of ["/chat", "/agent", "/stegvis", "/tiltakshjelpen", "/garasje"]) {
...
Object.assign(login, {
  ...
  krevEl: authEl, form: authEl("measure-form"),
  svg: { querySelector: authEl },
  utfylling: { cancel() {} },
  fetch: async () => { requests++; return Response.json({ feil: "Avvist forespørsel" }, { status: responseStatus }); }
});
const tiltakshjelpenSource = stripTypeScriptTypes(await readFile("apps/demo-gui/src/client/tiltakshjelpen.ts", "utf8"));
const apiStart = tiltakshjelpenSource.indexOf("function expireLogin");
const apiEnd = tiltakshjelpenSource.indexOf("async function perform", apiStart);
...
runInContext(`
  ...
  ${tiltakshjelpenSource.slice(apiStart, apiEnd)}
`, login);
for (const issuer of ["http://localhost:8086", config.idportenBaseUrl]) {
  ...
  assert.equal(authEl("measure-form").hidden, true);
...
const personStart = tiltakshjelpenSource.indexOf("async function loadPerson");
const personEnd = tiltakshjelpenSource.indexOf("function configureFields", personStart);
assert(personStart >= 0 && personEnd > personStart);
runInContext(`let embeddedOekt = null, adressevalg = []; ${tiltakshjelpenSource.slice(personStart, personEnd)}`, login);
...
const source = await readFile("apps/demo-gui/src/client/tiltakshjelpen-prosess.ts", "utf8");
runInContext(stripTypeScriptTypes(source).replace("export function mountTiltakshjelpenProsess", "function mountTiltakshjelpenProsess"), ui);
...
const dispose = runInContext("mountTiltakshjelpenProsess(options)", ui);
const frame = container.children[1];
const frameUrl = new URL(frame.src, "http://localhost:13001");
assert.equal(frameUrl.pathname, "/tiltakshjelpen");
assert.deepEqual(Object.fromEntries(frameUrl.searchParams), {
  integrert: "sporsmaal", oektsId: "garage-test", stegId: "garasje-prosjekt"
});
...
console.log("Tiltakshjelpens klient: innlogging først, person og eiendom, utløpt innlogging, skjerming og avgrenset kartkommunikasjon besto.");
