...
import { selectTiltakshjelpenProsessfelter } from "../apps/shared/tiltakshjelpen-dialog.ts";
...
const requiredFields = selectTiltakshjelpenProsessfelter(steps[2].felter!, "garasje");
...
const fenceFields = selectTiltakshjelpenProsessfelter(steps[2].felter!, "garasje", "gjerde");
...
try {
  ...
  await check("nytt navn, gamle navn, id og naturlig språk velger tiltakssjekken uten modell", async () => {
    for (const text of [TILTAKSSJEKK_NAVN, "Tiltakshjelpen", "tiltakshjelp", "Tiltakssjekken", "tiltakssjekk", "Byggesjekken",
  ...
  console.log(`\n${passed} agenttester for Tiltakshjelpen bestått.`);
} finally {
