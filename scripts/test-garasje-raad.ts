...
import { projectGarasjePlanflater } from "../apps/shared/garasje-kunnskap.ts";
...
for (const raad of ["Du kan bygge uten å søke.", "Du trenger ikke søke.", "Planbestemmelsene er kontrollert.", "Alle vilkår er oppfylt."]) {
  const result = kreves(validateGarasjeRaad({ antattUtfall: "maa_avklares", raad }, uavklart), raad);
...
assert.equal(validateGarasjeRaad({ raad: "x".repeat(1201) }, uavklart), null, "Lang prosa må ikke kuttes midt i et forbehold");
...
try {
  ...
  const projectedZones = projectGarasjePlanflater({
    punkt: { lat: 1, lon: 1 },
  ...
  const failedZoneAdvice = await post(agent, "/agent/garasje/raad", { kontekst: {
    resultater: { "garasje-vurdering": { vurdering: uavklart, grunnlag: {
      ...
      ...projectGarasjePlanflater({ planflater: [], kilder: [{ id: "planflater", status: "feil", merknad: "Kilden svarte ikke." }] })
    } } }
  } });
  ...
  console.log("Garasjeråd: agent, dokumentavgrensning, kildehenvisninger, mock og eksplisitte reservesvar besto.");
} catch (error) {
  console.error(logs);
  throw error;
} finally {
