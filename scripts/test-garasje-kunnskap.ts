...
import { buildGarasjeKunnskapsgrunnlag, projectGarasjeDokumentkunnskap, projectGarasjePlanflater } from "../apps/shared/garasje-kunnskap.ts";
import { projectGarasjeDialogGrunnlag } from "../apps/shared/garasje-dialog.ts";
import type { GarasjeGrunnlag, GarasjePlanflate } from "../apps/shared/garasje.ts";
import { retrieveGarasjeKunnskap } from "../apps/process-agent/src/garasje-kunnskap.ts";
...
for (const tiltakstype of ["gjerde", "tilbygg", "fasade", "ukjent"]) {
  const context = buildGarasjeKunnskapsgrunnlag({ prosjekt: { tiltakstype } });
...
assert.equal(buildGarasjeKunnskapsgrunnlag({ prosjekt: { tiltakstype: 123 } }).nasjonaleKrav, null);
assert.deepEqual(buildGarasjeKunnskapsgrunnlag({
  prosjekt: { tiltakstype: "gjerde", hoyde: 1.5, bya: 50, monehoyde: 4 }
}).prosjekt, { tiltakstype: "gjerde", hoyde: 1.5 });
assert.equal(buildGarasjeKunnskapsgrunnlag({
...
const retrieved = await retrieveGarasjeKunnskap(context, "byggegrense", async (name, args) => {
...
assert.equal(buildGarasjeKunnskapsgrunnlag({ ...context, ...retrieved }).planbestemmelserKontrollert, false);
const incomplete = projectGarasjeDokumentkunnskap([{ text: "tekst", page: -1, canonicalUrl: "javascript:alert(1)", checkRecommended: false }])[0];
...
const noScope = await retrieveGarasjeKunnskap({}, "spørsmål", async () => { throw new Error("Skal ikke gjøre bredt søk"); });
assert.match(noScope.kunnskapsadvarsel, /ukjent/);
const unavailable = await retrieveGarasjeKunnskap(context, "spørsmål", async () => { throw new Error("Planlagt testfeil"); });
assert.match(unavailable.kunnskapsadvarsel, /utilgjengelig/);
const noMatch = await retrieveGarasjeKunnskap({ resultater: { garasje: { adresse: { kommunenummer: "0301" } } } }, "plan", async () => ({
  dokumenter: [{ documentId: "bergen", status: "extracted", source: { canonicalUrl } }]
}));
assert.equal(noMatch.dokumentkunnskap.length, 0);
const provided = await retrieveGarasjeKunnskap(context, "plan", async name => name === "pdf_list_documents"
...
const diverse = await retrieveGarasjeKunnskap(diverseContext, "gjerde", async (name, args) => {
...
const missingLocal = await retrieveGarasjeKunnskap(diverseContext, "plan", async name => name === "pdf_list_documents"
...
const planflater: GarasjePlanflate[] = [
...
const zoneGround: GarasjeGrunnlag = {
...
const projectedZones = projectGarasjeDialogGrunnlag(zoneGround);
const zoneKnowledge = buildGarasjeKunnskapsgrunnlag({ resultater: projectedZones });
...
assert.deepEqual(zoneKnowledge.planflater, projectGarasjePlanflater(zoneGround).planflater, "Punktkontrollen bevares gjennom begge projeksjonene");
assert.doesNotMatch(JSON.stringify(projectedZones), /ringer|coordinates|IKKE_ADRESSE|"lat"|"lon"|vest=/);
assert.equal(projectGarasjePlanflater({ ...zoneGround, punkt: { lat: 0, lon: 1 } }).planflater[0].punktIFlate, true,
  "Punkt på sonegrensen er innenfor, som i felles geometrimodul");
const failedZones = buildGarasjeKunnskapsgrunnlag({ resultater: projectGarasjeDialogGrunnlag({
...
assert.equal(projectGarasjePlanflater({ ...zoneGround, kilder: [] }).planflater[0].punktIFlate, null);
const manyZones = projectGarasjePlanflater({ ...zoneGround,
...
assert.deepEqual(projectGarasjePlanflater(manyZones).planflatedekning, manyZones.planflatedekning);
