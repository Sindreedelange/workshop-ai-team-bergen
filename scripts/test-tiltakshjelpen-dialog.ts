...
import {
  FRITTLIGGENDE_DIALOGFELTER, isFrittliggendeDialogfeltId, normalizeQuestionFieldAnswer,
  validateFrittliggendeDialogHoyder, validateFrittliggendeDialogSvar, getByggetiltakDialogfelt, validateByggetiltakDialogSvar, selectTiltakshjelpenProsessfelter
} from "../apps/shared/tiltakshjelpen-dialog.ts";
import { BYGGETILTAK_KATALOG, TILTAKSSJEKK_NAVN } from "../apps/shared/byggetiltak.ts";
import type { FrittliggendeDialogfeltId } from "../apps/shared/tiltakshjelpen-dialog.ts";
...
const areaAnswers: [FrittliggendeDialogfeltId, string, number][] = [
  ["bra", "Bruksareal er 25 kvadratmeter", 25],
  ["bra", "Bruksarealet er 25,5 m².", 25.5],
  ["bra", "BRA: 25 kvm", 25],
  ["bra", "bra = +25 m2", 25],
  ["bya", " BEBYGD AREAL ER 25 KVADRATMETER. ", 25],
  ["bya", "BYA er 25", 25]
];
const invalidAreaAnswers: [FrittliggendeDialogfeltId, string][] = [
  ...[
    "Bruksareal er -25 kvadratmeter", "Bruksareal er −25 kvadratmeter",
    "Bruksareal er 0 kvadratmeter",
    "Bruksareal er 25 eller 30 kvadratmeter", "Bruksareal er 25-30 kvadratmeter",
    "Bruksareal er omtrent 25 kvadratmeter", "Bruksareal er under 25 kvadratmeter",
    "Bruksareal er ikke 25 kvadratmeter", "Bruksareal er 25 kvadratmeter eller mindre",
    "Bruksareal er 25 kvadratmeter og BYA er 30 kvadratmeter",
    "Bruksareal er 25 meter", "Bruksareal er 1e2 kvadratmeter",
    "Bruksareal er 2/3 kvadratmeter", "Bruksareal er 25,5,5 kvadratmeter",
    "Bebygd areal er 25 kvadratmeter",
    "Arealet er 25 kvadratmeter", "Jeg tror bruksareal er 25 kvadratmeter",
    "Bruksarealer 25 kvadratmeter", "Bruksareal er25 kvadratmeter"
  ].map(text => ["bra", text] as [FrittliggendeDialogfeltId, string]),
  ["bya", "Bruksareal er 25 kvadratmeter"],
  ["bya", "Bebygd areal er -25 kvadratmeter"],
  ["bya", "BYA er 0 m²"]
];

check("Entydige arealsetninger normaliseres likt i begge dialogene", () => {
  for (const [fieldId, text, value] of areaAnswers) {
    assert.deepEqual(validateFrittliggendeDialogSvar(fieldId, text), { valid: true, value }, text);
    assert.deepEqual(normalizeQuestionFieldAnswer({ id: fieldId, label: fieldId }, text, true),
      { valid: true, value: String(value) }, text);
  }
  assert.deepEqual(validateByggetiltakDialogSvar("bra", "Bruksareal er 0 kvadratmeter", "tilbygg"),
    { valid: true, value: 0 });
});
check("Arealsetninger må være positive, entydige og gjelde riktig felt", () => {
  for (const [fieldId, text] of invalidAreaAnswers) {
    assert.equal(validateFrittliggendeDialogSvar(fieldId, text).valid, false, text);
    assert.equal(normalizeQuestionFieldAnswer({ id: fieldId, label: fieldId }, text, true).valid, false, text);
  }
  assert.equal(validateFrittliggendeDialogSvar("bra", "Bruksareal er 10001 kvadratmeter").valid, false);
  assert.equal(validateFrittliggendeDialogSvar("bra", "Bruksareal er 25 kvadratmeter?").valid, false);
  assert.equal(validateByggetiltakDialogSvar("bra", "Bruksareal er -25 kvadratmeter", "tilbygg").valid, false);
});

check("Felles feltregister har elleve felt i skjemaets rekkefølge", () => {
  assert.deepEqual(FRITTLIGGENDE_DIALOGFELTER.map(field => field.id), [
    "bya", "bra", "gesimshoyde", "monehoyde", "etasjer", "avstandNabogrense",
    "avstandBygning", "frittliggende", "beboelse", "kjeller", "overVannAvlop"
  ]);
  assert.ok(FRITTLIGGENDE_DIALOGFELTER.every(field => field.label && field.hint));
  assert.ok(!isFrittliggendeDialogfeltId("__proto__"));
  assert.ok(!isFrittliggendeDialogfeltId("adresse"));
  assert.ok(!isFrittliggendeDialogfeltId("bebygdEiendom"));
  assert.ok(!isFrittliggendeDialogfeltId(null));
});
check("Valideringen bruker samme grenser for tekst og kanoniske verdier", () => {
  for (const field of FRITTLIGGENDE_DIALOGFELTER) {
    assert.equal(validateFrittliggendeDialogSvar(field.id, null).valid, field.ukjentTillatt);
    assert.equal(validateFrittliggendeDialogSvar(field.id, "vet ikke").valid, field.ukjentTillatt);
    for (const value of ["", [], {}, undefined, Infinity, NaN]) {
      assert.equal(validateFrittliggendeDialogSvar(field.id, value).valid, false, `${field.id}: ${String(value)}`);
    }
    if (field.type === "valg") {
      for (const [text, value] of [["ja", true], ["nei", false], ["vet-ikke", null], ["ukjent", null]] as const) {
        assert.deepEqual(validateFrittliggendeDialogSvar(field.id, text), { valid: true, value });
        assert.deepEqual(validateFrittliggendeDialogSvar(field.id, value), { valid: true, value });
      }
      for (const value of [0, 1, "ja eller nei", "ja men kanskje ikke", "true", "false"]) {
        assert.equal(validateFrittliggendeDialogSvar(field.id, value).valid, false);
      }
    } else {
      for (const value of [field.min!, field.max!]) {
        assert.deepEqual(validateFrittliggendeDialogSvar(field.id, value), { valid: true, value });
        assert.deepEqual(validateFrittliggendeDialogSvar(field.id, String(value)), { valid: true, value });
      }
  ...
  assert.equal(validateFrittliggendeDialogSvar("etasjer", 1.5).valid, false);
  assert.equal(validateFrittliggendeDialogSvar("bya", "3 meter").valid, false);
  assert.equal(validateFrittliggendeDialogSvar("gesimshoyde", "3 m²").valid, false);
  assert.equal(validateFrittliggendeDialogSvar("__proto__" as FrittliggendeDialogfeltId, "ja").valid, false);
});
check("Høyder sammenlignes uten å beregne et regelutfall", () => {
  assert.match(validateFrittliggendeDialogHoyder({ gesimshoyde: 4, monehoyde: 3 })!, /Gesimshøyden/);
  assert.equal(validateFrittliggendeDialogHoyder({ gesimshoyde: 3, monehoyde: 3 }), null);
  assert.equal(validateFrittliggendeDialogHoyder({ gesimshoyde: 3, monehoyde: 4 }), null);
  assert.equal(validateFrittliggendeDialogHoyder({ gesimshoyde: 3 }), null);
});
...
check("Prosessdialogen velger gammel garasje eller valgt tiltak, ikke alle valgfrie felt", () => {
  ...
  assert.deepEqual(selectTiltakshjelpenProsessfelter(fields, "garasje").map(field => field.id).sort(),
    ["adresse", ...FRITTLIGGENDE_DIALOGFELTER.map(field => field.id)].sort());
  for (const entry of BYGGETILTAK_KATALOG) {
    assert.deepEqual(selectTiltakshjelpenProsessfelter(fields, "garasje", entry.id).map(field => field.id).sort(),
      ["adresse", ...entry.sporsmaal.map(field => field.id)].sort());
  }
  assert.deepEqual(selectTiltakshjelpenProsessfelter(fields, "annen").map(field => field.id), ["adresse"]);
});
...
assert.doesNotMatch(await readFile(path.join(root, "apps/shared/tiltakshjelpen-dialog.ts"), "utf8"), /(?:from\s+|import\s*)["']node:/);
...
try {
  ...
  for (const [fieldId, text, value] of areaAnswers) {
    const result = await say(fieldId, text);
    assert.equal(result.type, "svar", text);
    assert.equal(result.svar, value, text);
    assert.ok(!Object.hasOwn(result, "modell"));
  }
  for (const [fieldId, text] of [...invalidAreaAnswers, ["bra", "Bruksareal er 10001 kvadratmeter"]]) {
    const result = await say(fieldId, text);
    assert.equal(result.type, "ugyldig", text);
    assert.ok(!Object.hasOwn(result, "svar"));
  }
  check("Arealsetninger tolkes og avvises deterministisk uten modell eller dokumentoppslag", () => {
  ...
  for (const text of [
    ...
    assert.deepEqual(context.aktivtFelt, {
      id: "gesimshoyde", label: FRITTLIGGENDE_DIALOGFELTER.find(field => field.id === "gesimshoyde")!.label
    });
  ...
  console.log(`Alle ${checks} kontroller av Tiltakshjelpens statsløse dialog bestod.`);
} catch (error) {
  console.error(logs);
  throw error;
} finally {
