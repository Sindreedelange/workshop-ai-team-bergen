 ...
 *
 * Formene fra tools-api er løsere enn kildens egne typer med vilje: agenten
 * videreformidler dem, den håndhever dem ikke.
 */
type Prosessvalg = { id: string; navn: string; beskrivelse?: string; antallSteg?: number };
...
type Prosessdefinisjon = { id: string; navn: string; steg?: Agentsteg[]; [felt: string]: unknown };

/*
 * Svarformene fra tools-api som agenten faktisk leser i. Løsere enn kildens
 * egne typer: agenten videreformidler dem til innbygger, den håndhever dem ikke.
 */
type FolkeregisterTreff = {
  foedselsEllerDNummer?: string;
  personnavn?: { fornavn?: string; mellomnavn?: string; etternavn?: string };
  bostedsadresse?: { kommune?: string };
  _sandbox?: { personId?: string };
};
...
type BrregOrg = {
  ...
  postadresse?: { kommune?: string; poststed?: string };
};

/** En gate fra matrikkel_finn_veger. */
type Matrikkelgate = {
  gateId?: string;
  adressenavn?: string;
  kommune?: string;
...
/** Modellens tolkning av et ja/nei-svar. */
type Tolkning = { intent?: string; confidence?: unknown; modell?: string; advarsel?: string };
...
/** Ett verktøyforslag fra suggest_step_tools. `bruk` sier når det skal kalles. */
...
type Stegresultat = {
  ...
  godkjent?: boolean;
  adressenavn?: string;
  funnetILokaltMatrikkel?: boolean;
  [felt: string]: unknown;
};

/** Svaret fra tools-api når et steg er utført. */
type Oektsvar = {
  oektsId?: string;
  sporingsId?: string;
  stegIndex?: number;
...
type Historikklinje = { role: string; message: string; tidspunkt?: string };

/** Et brukersvar etter tolkning: verdien, om den ble utledet, og om den holder. */
type Svartolkning = {
  /** null når svaret ikke lot seg tolke til en verdi. */
  answer: string | null;
  inferred: boolean;
  valid: boolean;
  note?: string | null;
...
type Agentsesjon = {
  ...
  created: string;
  updated: string;
  processes: Prosessvalg[];
  selectedProcess: Prosessvalg | null;
  processDefinition: Prosessdefinisjon | null;
  oektsId: string | null;
  ...
  latestMatrikkelGate: Record<string, unknown> | null;
...
function tokenizeForProcessChoice(text: unknown): string[] {
...
): { matches: number; userCount: number; processCount: number } {
...
function parseProcessChoice(text: string, processes: Prosessvalg[]): Prosessvalg | null {
  const value = normalize(text);
...
function formatProcessOptions(processes: Prosessvalg[], title: string): string {
  const lines = processes.map((process, index) => `${index + 1}. ${process.navn} (${process.id})`).join("\n");
  return [title, lines].join("\n\n");
}

// Kandidatene kommer fra modellen og har bare id og score.
type Kandidat = { id?: string; score?: unknown };

function mapCandidateProcesses(
  allProcesses: Prosessvalg[],
  candidates: Kandidat[] = []
): (Prosessvalg & { score?: unknown })[] {
  const byId = new Map((allProcesses || []).map((p) => [p.id, p]));
  return (candidates || [])
    .map((candidate) => {
      const process = candidate.id ? byId.get(candidate.id) : undefined;
    ...
    .filter((p) => p !== null) as (Prosessvalg & { score?: unknown })[];
}

function recentHistory(state: Agentsesjon, count = 8): { role: string; message: string }[] {
...
function parseNumberFromText(text: unknown): number | null {
...
function normalizeQuestionAnswer(stepId: string | null, text: string): Svartolkning {
...
const guidedInterviewDefinitions: Record<string, { key: string; question: string }[]> = {
  begrunnelse: [
    {
      key: "problem",
...
function composeGuidedAnswer(stepId: string | null, answers: Record<string, string>): string {
...
function looksLikeHelpQuestion(stepId: string | null, text: string): boolean {
...
function fallbackGuidanceForQuestion(stepId: string | null): string | null {
  if (stepId === "boliger-bekreft") {
...
function extractLookupCandidate(text: string) {
...
function isLikelyGateQuestionStep(step: Agentsteg | null | undefined): boolean {
  const allText = normalize(`${step?.id || ""} ${step?.tittel || ""} ${step?.tekst || ""}`);
  return allText.includes("gate") || allText.includes("gatenavn");
}

function extractPossibleGateMention(text: string): string | null {
  const value = String(text || "").trim();
  if (!value) return null;
...
function gateSearchText(input: unknown): string {
...
function extractBrregQuery(text: string) {
  let value = String(text || "").trim().replace(/[?]+$/g, "").trim();
  if (!value) return "";
...
// answer for it.
function extractPossibleFnr(text: string): string | null {
...
function extractFolkeregisterQuery(text: string) {
  let value = String(text || "").trim().replace(/[?]+$/g, "").trim();
  const quoted = value.match(/["'`""]([^"'`""]{2,})["'`""]/);
  if (quoted?.[1]) return quoted[1].trim();
...
async function maybeAnswerFolkeregisterQuestion(text: string): Promise<string | null> {
  ...
  if (fnr) {
    try {
      const person = await invokeTool<FolkeregisterTreff>("folkeregister_get_person", { foedselsEllerDNummer: fnr });
  ...
  try {
    const result = await invokeTool<{ personer?: FolkeregisterTreff[] }>("folkeregister_search_persons", { query, limit: 5 });
...
async function maybeAnswerBrregQuestion(text: string): Promise<string | null> {
  const lower = normalize(text);
  const mentionsBrreg = [
    "brreg",
    "enhetsregister",
  ...
  if (organisasjonsnummer) {
    try {
      const org = await invokeTool<BrregOrg>("brreg_get_organisation", { organisasjonsnummer });
  ...
  try {
    const result = await invokeTool<{ organisasjoner?: BrregOrg[] }>("brreg_search_organisations", { query, limit: 5, offset: 0 });
...
async function runValideringTool(toolName: string, userText: string) {
  if (toolName === "matrikkel_finn_veger") {
    try {
      ...
      const gateTreff = await invokeTool<Matrikkelgate[]>("matrikkel_finn_veger", { gate: query, all: true, limit: 10, offset: 0 });
...
async function moveSessionToStepId(state: Agentsesjon, targetStepId: string) {
...
function findProcessStepById(state: Agentsesjon, stepId: string): Agentsteg | null {
...
function maybeCaptureDeferredAnswer(state: Agentsesjon, currentStepId: string | null, text: string) {
  const value = String(text || "").trim();
...
function maybeAnswerProcessMetaQuestion(state: Agentsesjon, text: string): string | null {
  if (!text.includes("?")) return null;
  const lower = normalize(text);
  const step = state.lastSession?.aktivtSteg;
 ...
 */
function buildFlyt(state: Agentsesjon) {
...
/** Svaret på et sidespørsmål, med sperren som eventuelt slo til. */
type Sidesvar = { tekst: string; grunnlag?: unknown; sperre?: unknown };

async function maybeAnswerCitizenQuestion(state: Agentsesjon, text: string): Promise<Sidesvar | null> {
...
function looksLikeCitizenQuestion(text: string, streng = false): boolean {
  const lower = normalize(text);
...
function startGuidedInterview(state: Agentsesjon, stepId: string | null): string[] | null {
  const questions = (stepId && guidedInterviewDefinitions[stepId]) || [];
...
/** Modellens vurdering av hvilken prosess innbygger mener. */
type Prosessmatch = {
  intent?: string;
  prosessId?: string;
  confidence?: unknown;
...
async function resolveProcessChoiceWithAi(
  state: Agentsesjon,
  text: string,
  options: { streng?: boolean; processes?: Prosessvalg[] } = {}
) {
  ...
  const result = await invokeTool<Prosessmatch>("match_process_choice", {
    tekst: text,
...
async function startSelectedProcess(state: Agentsesjon, choice: Prosessvalg) {
  const started = await invokeTool<Oektsvar>("start_process_session", {
  ...
  try {
    state.processDefinition = await invokeTool<Prosessdefinisjon>("get_process_definition", { prosessId: choice.id });
  } catch {
    state.processDefinition = null;
  }
  state.oektsId = started.oektsId ?? null;
  state.sporingsId = started.sporingsId ?? null;
...
async function advanceAndPrompt(state: Agentsesjon): Promise<string[]> {
  ...
  while (true) {
    ...
    if (step.type === "DATA_FETCH") {
      const dataFetchResult = await invokeTool<Handlingssvar>("run_current_action", { oektsId: state.oektsId });
    ...
    if (step.type === "SJEKK") {
      const result = await invokeTool<Handlingssvar>("run_current_action", { oektsId: state.oektsId });
...
async function handleMessage(state: Agentsesjon, message: string): Promise<string[]> {
  ...
  if (!state.selectedProcess) {
    ...
    if (aiChoice.status === "matched" && aiChoice.process) {
  ...
  if (state.awaiting === "question") {
    ...
    if (!looksLikeHelpQuestion(activeQuestionId, text)) {
      ...
      if (isHelpish && valideringsToolName.length > 0) {
        ...
        for (const toolName of valideringsToolName) {
          const hint = await runKontekstTool(toolName, null);
    ...
    if (looksLikeHelpQuestion(activeQuestionId, text)) {
      ...
      return [
        fallbackGuidanceForQuestion(activeQuestionId) || "Beskriv gjerne situasjonen med dine egne ord.",
        step?.tekst || "Skriv gjerne svaret ditt når du er klar."
      ];
    }
    ...
    for (const toolName of valideringsToolName) {
      ...
      normalizedAnswer = {
        answer: valResult.answer ?? null,
        inferred: Boolean(valResult.inferred),
        valid: true,
        note: valResult.note
      };
  ...
  if (state.awaiting === "deferred_answer_confirm") {
    const intent = await invokeTool<Tolkning>("interpret_reply", {
      tekst: text,
  ...
  if (state.awaiting === "submit") {
    ...
    if (intent.intent === "submit_yes") {
      await invokeTool<Handlingssvar>("run_current_action", { oektsId: state.oektsId });
...
async function createAgentSession(body: { personId?: string }) {
  const personId = body.personId || "person-001";
  const processesResult = await invokeTool<{ prosesser: Prosessvalg[] }>("list_processes", {});

  const session: Agentsesjon = {
...
const server = createServer(async (request: IncomingMessage, response: ServerResponse) => {
  ...
  try {
    ...
    if (request.method === "POST" && url.pathname === "/agent/sessions") {
      const body = await readRequestBody(request) as { personId?: string };
