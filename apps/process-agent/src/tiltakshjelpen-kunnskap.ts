...
export async function retrieveTiltakshjelpenKunnskap(context: unknown, query: string, invoke: ToolCall) {
  const facts = buildTiltakshjelpenKunnskapsgrunnlag(context);
  ...
  try {
    ...
    const dokumentkunnskap = projectTiltakshjelpenDokumentkunnskap(diverse);
