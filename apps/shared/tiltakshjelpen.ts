...
export type TiltakshjelpenPunkt = { lat: number; lon: number };

export type TiltakshjelpenAdresse = {
  adressetekst: string;
  ...
  festenummer: number;
  undernummer: number;
  punkt: TiltakshjelpenPunkt;
};

export type TiltakshjelpenKilde = {
  id: string;
  navn: string;
  url: string;
  hentet: string;
...
export type TiltakshjelpenPolygon = {
  id: string;
  ringer: [number, number][][];
  teig?: TiltakshjelpenTeig;
  teigId?: number;
...
export type TiltakshjelpenArealformaal = {
  kode: number;
  beskrivelse: string;
  planId: string;
  arealstatus?: number;
...
export type TiltakshjelpenPlan = {
  planId: string;
  navn: string;
  url: string;
};

export type TiltakshjelpenEksisterendeBygning = {
  id: string;
...
export type TiltakshjelpenBebyggelse = {
  /** Bekrefter kartlagt eksisterende bebyggelse, aldri at den er lovlig. */
  status: "bekreftet" | "uavklart";
  bebygd: boolean | null;
  bygninger: TiltakshjelpenEksisterendeBygning[];
  forklaring: string;
  kilde: string;
};

export type TiltakshjelpenArealberegning = {
...
export type TiltakshjelpenNabotomter = {
  tomter: TiltakshjelpenPolygon[];
  kilde: TiltakshjelpenKilde;
};
...
export type TiltakshjelpenPlanflate =
  | (TiltakshjelpenPlanflateFelles & {
...
export type TiltakshjelpenGrunnlag = {
  adresse: TiltakshjelpenAdresse;
  punkt: TiltakshjelpenPunkt;
  arealformaal: TiltakshjelpenArealformaal[];
  reguleringsplaner: TiltakshjelpenPlan[];
  eiendomsgrenser: TiltakshjelpenPolygon[];
  eiendomsgeojson?: TiltakshjelpenEiendomsGeoJson;
  nabotomter?: TiltakshjelpenNabotomter;
  bygninger: TiltakshjelpenPolygon[];
  /** Hensynssoner og arealformål fra KPA2018 som berører eiendommen. */
  planflater: TiltakshjelpenPlanflate[];
  bebyggelse: TiltakshjelpenBebyggelse;
  arealberegning: TiltakshjelpenArealberegning;
  kilder: TiltakshjelpenKilde[];
  uavklarteForhold: string[];
  tiltaksvarsler?: TiltakshjelpenSjekk[];
};

export type FrittliggendeTiltak = {
  bra: number;
  bya: number;
  gesimshoyde: number;
  monehoyde: number;
...
export type TiltakshjelpenSjekk = {
  id: string;
  navn: string;
 ...
 *
 * `TILTAKSHJELPEN_UTFALL_FRITAR` er det ene utfallet som slipper innbyggeren fri fra å
 ...
 * klassifisere selv.
 */
export const TILTAKSHJELPEN_UTFALL = ["ikke_soknadspliktig", "meldeplikt", "soknadspliktig", "maa_avklares"] as const;
export type TiltakshjelpenUtfall = (typeof TILTAKSHJELPEN_UTFALL)[number];
export const TILTAKSHJELPEN_UTFALL_FRITAR = "ikke_soknadspliktig" as const;
 ...
 * uten å klassifisere selv. Klemmen i `tiltakshjelpen-raad.ts` leser den, og det er derfor
 ...
 */
const UTEN_SOKNAD: readonly TiltakshjelpenUtfall[] = ["ikke_soknadspliktig", "meldeplikt"];
export const utfallFritarForSoknad = (utfall: TiltakshjelpenUtfall): boolean => UTEN_SOKNAD.includes(utfall);

export type TiltakshjelpenVurdering = {
  tiltakstype?: Byggetiltakstype;
  utfall: TiltakshjelpenUtfall;
  nasjonaltUnntak: "oppfylt" | "brudd" | "uavklart";
  forklaring: string;
  sjekker: TiltakshjelpenSjekk[];
  uavklarteForhold: string[];
  nesteSteg?: string[];
};
 ...
 */
export type TiltakshjelpenSvarsetning = {
...
export function beskrivTiltakshjelpenUtfall(vurdering: TiltakshjelpenVurdering): TiltakshjelpenSvarsetning {
  ...
  const avgjorende = (status: TiltakshjelpenSjekk["status"]): string =>
  ...
  const setninger: Record<TiltakshjelpenUtfall, () => TiltakshjelpenSvarsetning> = {
    soknadspliktig: () => ({
 ...
 *
 * Her, ved siden av `TiltakshjelpenPlanflate`, fordi både regelen og kartet skriver
 ...
 */
export function hensynssonenavn(flate: Extract<TiltakshjelpenPlanflate, { kategori: "hensynssone" }>): string {
