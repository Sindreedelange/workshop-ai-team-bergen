...
export type TiltakshjelpenBegrep = {
  id: string;
  navn: string;
  forklaring: string;
  kilde: string;
...
export const TILTAKSHJELPEN_BEGREPER: readonly TiltakshjelpenBegrep[] = [
  {
    id: "gesimshoyde",
    navn: "Gesimshøyde",
...
export function isTiltakshjelpenKontekst(value: unknown): boolean {
...
export function findTiltakshjelpenBegreper(text: string): readonly TiltakshjelpenBegrep[] {
  ...
  return TILTAKSHJELPEN_BEGREPER.filter(begrep => ids.has(begrep.id));
}

export function buildTiltakshjelpenBegrepssvar(text: string, fieldId?: string): string | null {
  const matches = findTiltakshjelpenBegreper(text);
  const begreper = (matches.length ? matches : TILTAKSHJELPEN_BEGREPER.filter(begrep => begrep.id === fieldId)).slice(0, 2);
