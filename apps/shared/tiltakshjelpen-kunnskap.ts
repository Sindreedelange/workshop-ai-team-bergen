...
export function projectTiltakshjelpenPlanflater(value: unknown) {
...
export function projectTiltakshjelpenDokumentkunnskap(value: unknown) {
...
export function buildTiltakshjelpenKunnskapsgrunnlag(context: unknown = {}) {
  ...
  const kommune = kommunenummer ? findTiltakshjelpenKommunekilder(kommunenummer) : undefined;
  const prosjekt = projectTiltakshjelpenProsjekt(input.prosjekt);
  ...
  return {
    begreper: frittliggende ? TILTAKSHJELPEN_BEGREPER : TILTAKSHJELPEN_BEGREPER.filter(begrep => relevantFields.has(begrep.id)),
    nasjonaleKrav: frittliggende ? FRITTLIGGENDE_NASJONALE_KRAV : null,
    ...
    tiltak: tiltak ? {
      ...
      kilde: FRITTLIGGENDE_NASJONALE_KRAV.kilde,
    ...
    kommunenummer,
    kommunaleOppsett: Object.values(TILTAKSHJELPEN_KOMMUNER).map(kommune => ({
    ...
    sonetyper: listTiltakshjelpenSonetyper(),
    ...
    dokumentkunnskap: projectTiltakshjelpenDokumentkunnskap(input.dokumentkunnskap),
    ...
    regelutfall: (TILTAKSHJELPEN_UTFALL as readonly string[]).includes(String(vurdering.utfall))
...
export function projectTiltakshjelpenProsjekt(value: unknown): Record<string, number | boolean | string | null> {
  const input = record(value);
