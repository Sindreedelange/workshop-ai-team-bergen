 ...
 *
 * Eierforholdet hører til grunnboken, ikke matrikkelen, og slås på her fra
 * data/eierforhold.json. Se kommentaren over readEierforhold.
 */
type Koordinater = { lat: number; lon: number; epsg: string };

/** En hjemmelshaver med sin andel, slik grunnboken fører det. */
type Eierandel = { eier: string; eierform?: string; andel?: string };
...
type Eiendom = {
  ...
  adressekode: number | string | null;
  postnummer: string;
  poststed: string;
  koordinater: Koordinater | null;
  adressetilleggsnavn: string | null;
  objtype: string;
  bruksenhetstype?: string;
  /** Flat liste av eier-id-er. Det er denne formen alle konsumenter leser. */
  eiere?: string[];
  eierforhold?: Eierandel[];
  syntetisk?: boolean;
  /** Bare på eiendommer utledet fra Geonorge, ikke fra seeden. */
  kilde?: { navn: string; type: string };
};
...
type Register = {
  gater: Gate[];
  gaterPerId: Map<string, Gate>;
  gaterPerNormalisertNavn: Map<string, Gate[]>;
  eiendommer: Eiendom[];
  eiendomPerId: Map<string, Eiendom>;
  eiendomPerGnrBnr: Map<string, string[]>;
  eiendomPerNormalisertAdresse: Map<string, string[]>;
  eiendomPerAdressePrefix: Map<string, string[]>;
  kilde: Kilde;
  lastetTidspunkt: string;
  eierforhold?: { fil: string | null; antall: number };
};
...
function safeNumber(verdi: unknown, fallback = 0): number {
...
function normalizeAdresse(verdi: unknown): string {
  return normalize(verdi).replace(/\s+/g, " ").trim();
}

function createEmptyRegister(kilde: Kilde): Register {
  return {
    gater: [],
    gaterPerId: new Map<string, Gate>(),
    gaterPerNormalisertNavn: new Map<string, Gate[]>(),
    eiendommer: [],
    eiendomPerId: new Map<string, Eiendom>(),
    eiendomPerGnrBnr: new Map<string, string[]>(),
    eiendomPerNormalisertAdresse: new Map<string, string[]>(),
    eiendomPerAdressePrefix: new Map<string, string[]>(),
    kilde,
...
async function readEierforhold(): Promise<{ fil: string | null; perMatrikkelId: Map<string, Eierandel[]> }> {
  const kandidatfiler = [
  ...
  for (const fil of kandidatfiler) {
    try {
      const json = JSON.parse(await readFile(fil, "utf8")) as {
        eierforhold?: { matrikkelId: string; eiere?: Eierandel[] }[];
      };
      const perMatrikkelId = new Map<string, Eierandel[]>();
      for (const rad of json.eierforhold || []) {
        perMatrikkelId.set(rad.matrikkelId, rad.eiere || []);
      }
      return { fil, perMatrikkelId };
    } catch (error) {
      if (feilkode(error) !== "ENOENT") throw error;
    }
  }
...
function getGateNoekkel(gate: Partial<Gate> | undefined): string {
...
function getOrCreateGate(register: Register, gateInput: Partial<Gate> = {}): Gate {
  const noekkel = getGateNoekkel(gateInput);
  const funnet = register.gaterPerId.get(noekkel);
  if (funnet) {
    return funnet;
  }
...
function normalizeText(verdi: unknown): string {
  return String(verdi || "")
    .normalize("NFKD")
    .replace(/\p{M}/gu, "")
    .toLowerCase()
...
function addBonesheienIfMissing(register: Register): void {
...
function findEiendomViaLive(register: Register, adresseSoek: string) {
...
async function findGaterViaLive(gateSoek: string, includeEiendommer = false, kommunenummer: string | null = null) {
  try {
...
// SOAPAction i Allow-Headers er denne tjenestens egen: den er den eneste med en
// SOAP-flate, og uten headeren dør en SOAP-forespørsel fra nettleseren i preflight.
const { jsonResponse, textResponse: sendTekst } = svarhjelpere({
  cors: cors("GET,POST,OPTIONS", "Content-Type,SOAPAction,Authorization")
});
...
function xmlEscape(verdi: unknown): string {
  return String(verdi)
    .replaceAll("&", "&amp;")
    .replaceAll("<", "&lt;")
    .replaceAll(">", "&gt;")
...
function findTag(xml: string, taggnavn: string): string | null {
  const treff = xml.match(new RegExp(`<(?:\\w+:)?${taggnavn}[^>]*>([\\s\\S]*?)<\\/(?:\\w+:)?${taggnavn}>`, "i"));
  return treff ? treff[1].trim() : null;
}

function findOperation(xml: string): string | null {
...
function buildMatrikkelNoekkel(eiendom: Eiendom): string {
  return `${safeNumber(eiendom.gnr, -1)}:${safeNumber(eiendom.bnr, -1)}`;
}

function addEiendom(register: Register, gate: Gate, eiendomInput: Partial<Eiendom> = {}): void {
...
function gateSomRespons(register: Pick<Register, "eiendomPerId">, gate: Gate, includeEiendommer = false) {
  const base = {
    gateId: gate.gateId,
    adressenavn: gate.adressenavn,
    kommunenummer: gate.kommunenummer,
  ...
  return {
    ...
    eiendommer: gate.eiendomIds
      .map((id) => register.eiendomPerId.get(id))
      .filter((eiendom): eiendom is Eiendom => Boolean(eiendom))
  };
}
...
function findGate(register: Register, gateSoek: string): Gate | null {
...
function findEiendomFraAdresse(register: Register, adresseSoek: string): Eiendom | null {
...
/** Eiendommen med alt utledet fylt inn. Gatefeltene settes av kallstedet. */
type BeriketEiendom = Partial<Eiendom> & Pick<Eiendom,
  "adresse" | "husnummer" | "husbokstav" | "festenummer" | "undernummer"
...
function normalizeGeonorgeSoek(verdi: unknown): string {
...
function pickBestLiveAdresse(adresser: GeonorgeAdresse[], query: string): GeonorgeAdresse | null {
  ...
  const kandidatTekst = (adresse: GeonorgeAdresse) => normalize(geonorgeAdresseTekst(adresse)).replace(/\s+/g, " ").trim();
  const kandidatKjerne = (adresse: GeonorgeAdresse) => buildAdresseKjerne(geonorgeAdresseTekst(adresse));
...
function geonorgeAdresseTilGate(adresse: GeonorgeAdresse): Partial<Gate> {
  return {
...
function buildLiveGateTreff(adresser: GeonorgeAdresse[], includeEiendommer = false): LiveGate[] {
  ...
  return [...perGate.values()].map((gate) => ({
    ...gate,
    eiendommer: includeEiendommer
      ? gate.eiendommer.sort((a, b) => String(a.adresse ?? "").localeCompare(String(b.adresse ?? ""), "nb"))
      : []
...
type GateRespons = Pick<Gate, "gateId" | "adressenavn" | "kommunenummer" | "kommune" | "postnummer" | "poststed">
  & { antallEiendommer?: number; antallBoligeiendommer?: number };

function buildGateReturn(gate: GateRespons): string {
  return [
    "      <return>",
...
/** Feltene buildEiendomReturn trenger fra gata. Alle tre kallstedene sender nettopp disse. */
type Gatekjerne = Pick<Gate, "adressenavn" | "kommunenummer" | "kommune" | "postnummer" | "poststed">
  & { gateId?: string };

function buildEiendomReturn(gate: Gatekjerne, eiendom: Partial<Eiendom>): string {
...
function handleSoapRequest(operasjon: string | null, xml: string, register: Register): string {
  ...
  if (operasjon === "FinnMatrikkelenheter") {
    ...
    const eiendommer = gater.flatMap((gate) =>
      gate.eiendomIds.map((id) => register.eiendomPerId.get(id)).filter((e): e is Eiendom => Boolean(e)));
...
function handleSoap(operasjon: string | null, xml: string, matrikkel: Register): string {
    ...
    if (request.method === "POST" && url.pathname === wsPath) {
      ...
      if (operasjon === "FinnMatrikkelenheter") {
        ...
        const treff = gater.length > 0
          ? gater.flatMap((gate) => gate.eiendomIds.map((id) => register.eiendomPerId.get(id)).filter((e): e is Eiendom => Boolean(e)).map((eiendom) => ({
              gateId: gate.gateId,
              adressenavn: gate.adressenavn,
              kommunenummer: gate.kommunenummer,
              kommune: gate.kommune,
