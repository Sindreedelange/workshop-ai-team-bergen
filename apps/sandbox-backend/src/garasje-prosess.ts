import type { GarasjeTiltak } from "../../shared/garasje.ts";
import { HttpError } from "./errors.ts";
import { validateGarasjeTiltak } from "./garasje.ts";
import { readGarasjeRequest, readGarasjeKommune } from "./garasje-request.ts";
...
export function normalizeGarasjeSvar(input: unknown): Record<string, string | number> {
  if (!input || typeof input !== "object" || Array.isArray(input)
    || Object.keys(input).some(key => !allowedFields.includes(key))) {
    throw new HttpError("Garasjeprosjektet må bare inneholde de dokumenterte svarfeltene, ikke planvurderinger eller datagrunnlag.", 400);
  }
  ...
  if (fields.kommunenummer !== undefined) {
    svar.kommunenummer = readGarasjeKommune(new URLSearchParams({ kommunenummer: String(fields.kommunenummer) }))!;
  }
  ...
  readGarasjeRequest(buildGarasjeSok(svar));
  buildGarasjeTiltak(svar);
  return svar;
}

export function buildGarasjeSok(svar: Record<string, string | number>): URLSearchParams {
  return new URLSearchParams(Object.fromEntries(
    ["adresse", "gnr", "bnr", "lat", "lon", "kommunenummer"].filter(field => svar[field] !== undefined).map(field => [field, String(svar[field])])
  ));
}

export function buildGarasjeTiltak(svar: Record<string, string | number>): GarasjeTiltak {
  return validateGarasjeTiltak({
    bebygdEiendom: null,
