...
import { getTiltakshjelpenGrunnlag, searchTiltakshjelpenAdresser } from "./tiltakshjelpen-data.ts";
...
export async function readTiltakshjelpenGrunnlag(sok: URLSearchParams) {
  ...
  const grunnlag = await getTiltakshjelpenGrunnlag(adresse, request.plassering);
