...
function parsePlansoner(
  raw: unknown, kategori: GarasjePlanflate["kategori"]
  ...
  const flater = data.features.map(raw => {
    ...
    return {
      ...
      geometry: { type: "Polygon" as const, coordinates: (record(feature.geometry).coordinates ?? []) as number[][][] },
      properties: {
 ...
 * uten kryss betyr «helt». Det som blir igjen er flater som verken krysser eller
 * omslutter, og bare der er den dyre planimetrireserven inne i `polygonsIntersect`
 * verdt å betale for.
 */
  ...
  const [arealformaal, reguleringsplaner, eiendomsgrenser, bygninger, nabotomter, planflater] = await Promise.all([
    ...
    load<GarasjePlanflate>("planflater", async () => {
      ...
      const [soner, formaal] = await Promise.all([
        readJson(soneUrl, "Planmockens hensynssone-API").then(raw => parsePlansoner(raw, "hensynssone")),
        readJson(planQuery("arealformaal", adresse.kommunenummer, bounds), "Planmockens arealformål-API")
          .then(raw => parsePlansoner(raw, "arealformaal")),
      ]);
