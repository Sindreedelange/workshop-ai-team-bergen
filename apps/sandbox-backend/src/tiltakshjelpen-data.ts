...
import { containsPunkt, validateTiltakshjelpenPunkt } from "./tiltakshjelpen.ts";
...
function officialLayerUrl(id: LayerId, kommune: TiltakshjelpenKommunekilder | undefined): string {
...
async function queryLayer(id: LayerId, kommune: TiltakshjelpenKommunekilder | undefined, p: TiltakshjelpenPunkt, geometry: boolean, envelope?: Envelope): Promise<Record<string, unknown>[]> {
...
function lokalTeigQuery(adresse: TiltakshjelpenAdresse): URL {
...
function parseLokaleTeiger(input: unknown, adresse: TiltakshjelpenAdresse, nabotomter = false): {
  geojson: TiltakshjelpenEiendomsGeoJson;
...
function polygonsIntersect(a: TiltakshjelpenPolygon, b: TiltakshjelpenPolygon): boolean {
  const strictlyInside = (p: Pair, shape: TiltakshjelpenPolygon) => containsPunkt({ lon: p[0], lat: p[1] }, shape)
    && !shape.ringer.some(ring => ring.slice(1).some((end, i) => segmentsIntersect(p, p, ring[i]!, end)));
  const hasInsidePoint = (shape: TiltakshjelpenPolygon, target: TiltakshjelpenPolygon) => shape.ringer.some(ring =>
  ...
  try {
    return calculateTiltakshjelpenAreal([a], [b], { lon: (first[0] + first[2]) / 2, lat: (first[1] + first[3]) / 2 }).kartlagtBebygdArealM2 > 1e-6;
  } catch (error) {
    if (!(error instanceof HttpError)) throw error;
    return false;
  }
...
function polygonEnvelope(polygon: TiltakshjelpenPolygon): Envelope {
...
export function calculateTiltakshjelpenAreal(parcels: TiltakshjelpenPolygon[], buildings: TiltakshjelpenPolygon[], origin: TiltakshjelpenPunkt) {
  ...
  const edgesFor = (polygons: TiltakshjelpenPolygon[]): Segment[][] => polygons.map(polygon => polygon.ringer.flatMap(ring => {
...
function buildArealberegning(
  ...
  try {
    const parcelArea = calculateTiltakshjelpenAreal(parcels, [], adresse.punkt);
 ...
 * utsnitt her ville gitt flater kartet ikke viser og omvendt.
 */
function planUtsnitt(parcels: TiltakshjelpenPolygon[], p: TiltakshjelpenPunkt): Kartutsnitt {
...
const PLANDATASETT: Record<TiltakshjelpenPlanflate["kategori"], readonly string[]> = {
  arealformaal: ["arealformaal"],
  hensynssone: DATASETTIDER.filter(id => id !== "arealformaal"),
};

function parsePlansoner(
  raw: unknown, kategori: TiltakshjelpenPlanflate["kategori"], kommunenummer: string
): { flater: PlansoneFeature[]; kilde: Partial<TiltakshjelpenKilde>; dekket: boolean } {
 ...
 */
function beroring(flate: TiltakshjelpenPolygon, parcels: TiltakshjelpenPolygon[], flateBounds: Kartutsnitt): "helt" | "delvis" | null {
...
function byggPlanflater(
  flater: PlansoneFeature[], kategori: TiltakshjelpenPlanflate["kategori"], parcels: TiltakshjelpenPolygon[], bounds: Kartutsnitt
): TiltakshjelpenPlanflate[] {
  ...
  for (const feature of flater) {
    ...
    const polygon: TiltakshjelpenPolygon = { id: `${e.datasett}-${feature.id}`, ringer };
...
async function getNabotomter(adresse: TiltakshjelpenAdresse, parcels: TiltakshjelpenPolygon[]): Promise<TiltakshjelpenNabotomter> {
...
export async function getTiltakshjelpenGrunnlag(adresse: TiltakshjelpenAdresse, plassering?: TiltakshjelpenPunkt): Promise<TiltakshjelpenGrunnlag> {
  ...
  const eiendomPromise = load<TiltakshjelpenPolygon>("eiendomsgrenser", async () => {
  ...
  const [arealformaal, reguleringsplaner, eiendomsgrenser, bygninger, nabotomter, planflater] = await Promise.all([
    load<TiltakshjelpenArealformaal>("kpa", async () => (await queryLayer("kpa", kommune, p, false)).map(feature => {
      const attrs = record(feature.attributes);
    ...
    load<TiltakshjelpenPlan>("reguleringsplan", async () => (await queryLayer("reguleringsplan", kommune, p, false)).map(feature => {
    ...
    load<TiltakshjelpenPlanflate>("planflater", async () => {
      const parcels = await eiendomPromise;
