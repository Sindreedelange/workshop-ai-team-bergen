...
export type Sporsmaalskontekst = {
  ...
  prosessId?: string;
  prosess?: unknown;
  steg?: { tittel?: string; type?: string; [felt: string]: unknown };
  satser?: { gjelderFra?: string; [felt: string]: unknown };
  /** Hvor i prosessen innbygger står. Se buildFlyt hos kallstedene. */
  flyt?: {
    soknadSendt?: boolean;
    staarPaa?: string | null;
    isOnList?: string | null;
    [felt: string]: unknown;
  };
  samtykke?: { status?: string; [felt: string]: unknown };
  /** Resultatet av hvert utført steg, nøklet på stegId. Projisert av sanitize. */
  resultater?: Record<string, Record<string, unknown>>;
...
/** Utfallet av en sperre. `ok: false` betyr at svaret ble erstattet. */
export type Sperreutfall = {
  ok: boolean;
  sperre?: string;
  advarsel?: string;
  tekst?: string;
 ...
 */
function foldNorwegian(tekst: string): string {
  return tekst
    .replace(/[æ]/g, "ae")
    .replace(/[øö]/g, "o")
    .replace(/[åä]/g, "a");
...
function findFalseCompletionClaims(tekst: string, kontekst: Sporsmaalskontekst | null | undefined): string[] {
...
const GARASJEBESLUTNINGER = [
  ...
  "garasjen er lovlig", "garasjen er tillatt", "du slipper a soke", "du kan bygge",
  "tiltaket er soknadspliktig", "garasjen er soknadspliktig",
  "tillatt utnyttelse er", "tillatt utnyttelsesgrad er", "utnyttelsesgrensen er"
];
...
function collectNumbers(verdi: unknown, ut = new Set<number>()): Set<number> {
...
export function buildGrunnlagsIndeks(kontekst: unknown): Set<number> {
  const raa = collectNumbers(kontekst);
  const lovlige = new Set<number>();
...
function findLeakedIdentifiers(tekst: string, kontekstTekst: string): string[] {
  const funn = [];
...
export function hasInjeksjonsmarkorer(tekst: unknown): boolean {
...
export function buildGarasjeVeiledningssvar(tekst: string, kontekst: Sporsmaalskontekst): string | null {
  ...
  if (knowledge && isGarasjeRegelsporsmaal(foldNorwegian(normalizeText(tekst)))) {
    ...
    return [
      "Dette er tallkrav i det nasjonale unntaket, ikke en tillatelse til å bygge:",
      ...(selected.length ? selected : all).map(([, krav]) =>
        `${krav.navn}: ${krav.operator === "maks" ? "høyst" : krav.operator === "min" ? "minst" : "nøyaktig"} ${krav.verdi} ${krav.enhet}.`),
      "Alle øvrige vilkår må også være oppfylt. Planbestemmelser og andre krav for eiendommen må avklares.",
...
export function isPersonvernSporsmaal(sporsmaal: unknown): boolean {
  const tekst = foldNorwegian(normalizeText(sporsmaal));
  return PERSONVERNTEMA.some((tema) => tekst.includes(foldNorwegian(tema)));
}

export function buildPersonvernSvar(kontekst: Sporsmaalskontekst | null | undefined): string {
...
export function validateAnswer(tekst: unknown, kontekst: Sporsmaalskontekst | null | undefined): Sperreutfall {
  ...
  if (isGarasjeKontekst(kontekst)) {
    ...
    for (const match of svar.matchAll(/\b(\d+(?:[.,]\d+)?)\s*(m²|m2\b|kvadratmeter\b|meter\b|cm\b|m\b)/gi)) {
      if (!maalgrunnlag.has(Number(match[1].replace(",", ".")))) udekkede.push(match[0]);
    }
  }
...
export function sanitizeSporsmaalKontekst(kontekst: unknown): Sporsmaalskontekst {
  ...
  if (inn.steg) {
    ...
    if (inn.steg.visning === "garasje") ut.steg.visning = "garasje";
  }
  ...
  if (!garasje && inn.resultater && typeof inn.resultater === "object") {
    ...
    for (const [stegId, raa] of Object.entries(inn.resultater as Record<string, unknown>)) {
      ...
      // Projeksjonen under er hele poenget: bare utfallet slipper gjennom, aldri
      // grunnlaget det ble bygget av. Formen navngir det som slippes.
      const resultat = raa as {
        godkjent?: unknown;
        melding?: unknown;
        grunnlag?: unknown;
        tekst?: unknown;
      ...
      const projeksjon: Record<string, unknown> = {};
 ...
 */
export function buildGrunnlag(kontekst: Sporsmaalskontekst): { kilder: string[]; verdier: Sporsmaalskontekst } {
  const kilder: string[] = [];
  if (kontekst.satser) {
