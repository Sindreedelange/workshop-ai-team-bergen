...
/** Ett felt på et QUESTION-steg, slik promptbyggeren leser det. */
type Stegfelt = { id?: string; label?: string; placeholder?: string };

/** Søkerens eiendommer, hentet av kallstedet og lagt ved som grunnlag. */
type MineEiendommer = {
  eiendommer?: { adresse?: string; bruksenhetstype?: string; kommune?: string }[];
};

/** Felles kropp for alle /ai-rutene. `kontekst` er det modellen får se. */
type AiKropp = {
  tekst?: string;
  sporingsId?: string;
  sprak?: string;
  /** Sporsmaalskontekst har en åpen indeks, så data/svar/steg dekkes av den. */
  kontekst?: Sporsmaalskontekst;
  jaIntent?: string;
  neiIntent?: string;
  ukjentIntent?: string;
  prosesser?: { id?: string; navn?: string; beskrivelse?: string }[];
  history?: { role?: string; message?: string }[];
  /** Enten bare navn, eller navn med beskrivelse. Begge formene forekommer. */
  verktoy?: (string | { name?: string; navn?: string; description?: string; beskrivelse?: string })[];
  steg?: Record<string, unknown>;
  [felt: string]: unknown;
};

/** Én linje i KI-sporet. Skrives til state/ai-trace.jsonl, lest av GET /trace. */
type Sporlinje = {
  timestamp: string;
...
// for å svare uten å tenke: en oppgave som ser ut som den tenker uten å gjøre det er
// verre enn en som sier at den ikke kan.
const reasoningValg = velgReasoningModell(AI_FACTORY_MODELS, process.env.TELENOR_AI_FACTORY_REASONING_MODEL);
const aiFactoryReasoningModel = reasoningValg.modell ?? aiFactoryModel;
if (reasoningValg.advarsel) {
  console.warn(`Reasoning-modell: ${reasoningValg.advarsel}`);
}
// Endepunktet ligger bak en API Gateway som kutter forbindelsen etter 30 sekunder
// og svarer «Service Unavailable». AI_TIMEOUT_MS er 180000, så uten dette taket
...
async function readTrace(
  { limit = 50, sporingsId = null, task = null }:
    { limit?: number; sporingsId?: string | null; task?: string | null } = {}
): Promise<Sporlinje[]> {
  let raw;
  try {
...
function traceHtml(
  entries: Sporlinje[],
  filter: { sporingsId?: string | null; task?: string | null } = {},
  alle: Sporlinje[] = entries
): string {
      ...
      function renderChoices(data, valgtProvider) {
        ...
        document.getElementById("aiFactoryCreds").textContent = data["telenor-ai-factory"].keyConfigured
          ? "Base-URL: " + data["telenor-ai-factory"].baseUrl
            + " · Reasoning-oppgaver bruker " + data["telenor-ai-factory"].reasoningModel
            + " · Taket er " + data["telenor-ai-factory"].timeoutMs + " ms"
...
function buildTemplateResponse(type: string, body: AiKropp) {
  const tjeneste = body?.kontekst?.tjeneste || "kommunal tjeneste";
  const data = body?.kontekst?.data || {};
  const svar = (body?.kontekst?.svar || {}) as Record<string, unknown>;
   ...
   */
  function findValue(predicate: (verdi: any) => boolean): any {
    return Object.values(data || {}).find(predicate) || null;
  }

  function formatBelop(tall: unknown): string {
  ...
  function buildHusstandLines() {
    ...
    const foresatte = husstand.medlemmer.filter((m: any) => m.rolle === "foresatt");
    const barn = husstand.medlemmer.filter((m: any) => m.rolle === "barn");
    const chunks = [`Husstand: ${husstand.adresse || husstand.husstandId}`];
    if (foresatte.length) chunks.push(`${foresatte.length} foresatt${foresatte.length !== 1 ? "e" : ""} (${foresatte.map((m: any) => m.personId).join(", ")})`);
    if (barn.length) chunks.push(`${barn.length} barn (${barn.map((m: any) => m.personId).join(", ")})`);
    return chunks.join(", ");
  }
  ...
  function buildInntektLines() {
    ...
    const poster = (beregning.visningsposter || [])
      .flatMap((v: any) => v.poster || [])
      .map((p: any) => `${p.visningstekst} ${formatBelop(p.beloep)} kr`)
      .join(", ");
    const utenfor = (beregning.fradrag?.beregning || [])
      .flatMap((g: any) => g.beregningsposter || [])
      .map((p: any) => p.visningstekst)
      .join(", ");
  ...
  return {
    tekst: tekster[type as keyof typeof tekster],
    syntetisk: true,
...
// Tool selection - which tools are relevant for a given process step
...
function heuristicToolChoice(body: AiKropp) {
  ...
  const alleStegTekster = normalizeText(
    [steg.id, steg.tittel, steg.tekst, ...(felter as Stegfelt[]).map((f) => `${f.id || ""} ${f.label || ""} ${f.placeholder || ""}`)].join(" ")
  );
  ...
  for (const regel of TOOL_HEURISTICS) {
    if (!tilgjengeligeVerktoy.some((v) => (typeof v === "string" ? v : v.name) === regel.navn)) continue;
...
function buildToolChoicePrompt(body: AiKropp): string {
  const steg = body?.steg || {};
  ...
  const verktoyTekst = verktoyListe
    .map((v) => (typeof v === "string" ? `- ${v}: ` : `- ${v.name || ""}: ${v.description || ""}`))
    .join("\n");
...
function validateToolChoice(data: unknown, verktoyNavn: string[]) {
...
async function chooseToolsWithAi(body: AiKropp) {
  ...
  try {
    ...
    return {
      ...
      advarsel: `LLM-verktøyvalg feilet: ${feilmelding(error)}`
    };
  }
}
...
function buildPrompt(type: string, body: AiKropp, fallbackTekst: string): string {
  ...
  if (type === "sporsmaal") {
    sperrer.push(
      ...
      "I byggesjekken gjelder nasjonaleKrav bare tiltakstypen angitt i nasjonaleKravGjelderTiltakstype. Ikke bruk garasjens areal- eller høydegrenser for gjerde, tilbygg, fasade eller ukjent tiltak.",
  ...
  if (type === "sporsmaal") {
    ...
    const eiendommer = (kontekst.mineEiendommer as MineEiendommer | undefined)?.eiendommer;
...
function stemProcessToken(token: string): string {
...
function tokenizeProcessText(tekst: unknown): string[] {
...
function sharedTokenMatches(
  brukerToken: string[],
  prosessToken: string[]
): { treff: number; brukerAntall: number; prosessAntall: number } {
...
function heuristicProcessChoice(body: AiKropp) {
  ...
  const scoredeKandidater = prosesser
    ...
    .filter((kandidat) => kandidat !== null)
    .sort((a, b) => b!.score - a!.score) as { id?: string; navn?: string; score: number }[];
...
function buildProcessChoicePrompt(body: AiKropp): string {
...
/** Modellens prosessvalg etter validering. */
type Prosessvalgsvar = {
  intent: string;
  prosessId?: string | null;
  confidence: number;
  begrunnelse?: string;
  kandidater?: { id?: string; navn?: string; score?: number }[];
};
...
function validateProcessChoice(raa: unknown, body: AiKropp): Prosessvalgsvar | null {
  ...
  if (!raa || typeof data.intent !== "string" || !gyldigeIntent.has(data.intent)) {
    return null;
  }
  ...
  const kandidater = Array.isArray(data.kandidater)
    ? data.kandidater
      .filter((k: { id?: string }) => k && gyldigeProsessIder.has(k.id))
      .map((k: { id?: string; score?: unknown }) => ({
        id: k.id,
...
function hasNegation(tekst: string): boolean {
  return normalizeText(tekst).split(" ").some((ord) => negations.includes(ord));
}

function heuristicIntent(body: AiKropp) {
  ...
  const treffer = (monstre: string[]) => monstre.some((monster) => containsPhrase(ord, monster));
...
function buildIntentPrompt(body: AiKropp): string {
  const jaIntent = body?.jaIntent || "ja";
  const neiIntent = body?.neiIntent || "nei";
  const ukjentIntent = body?.ukjentIntent || "ukjent";
  return [
...
function parseJsonObject(tekst: string): Record<string, unknown> | null {
...
function validateIntent(raa: unknown, body: AiKropp): Intentsvar | null {
  // Rå JSON fra modellen; typen navngir bare feltene som leses.
  const data = (raa || {}) as { intent?: unknown; confidence?: unknown; begrunnelse?: unknown };
  const jaIntent = body?.jaIntent || "ja";
  const neiIntent = body?.neiIntent || "nei";
  const ukjentIntent = body?.ukjentIntent || "ukjent";
  const gyldige = new Set([jaIntent, neiIntent, ukjentIntent]);

  if (!raa || typeof data.intent !== "string" || !gyldige.has(data.intent)) {
    return null;
  }
...
async function callOpenRouter(prompt: string, temperature: number, systemMessage: string, signal: AbortSignal): Promise<Modellsvar> {
  if (!openRouterApiKey) {
...
/**
 * Reasoning styres med `chat_template_kwargs.enable_thinking`, ikke med
 * `reasoning_effort`: LiteLLM avviser det siste med 400 og
 * «openai does not support parameters: ['reasoning_effort']» med mindre kallet
 * også sender `allowed_openai_params`. Flagget sendes alltid, også når det er
 * `false`, fordi GLM-5.2 og Nemotron begge tenker som standard.
 *
 * Merk at det ikke finnes noe `max_tokens` her. Taket gjelder reasoning-tokenene
 * også, så et lavt tak spiser hele budsjettet på tenkingen og lar `content` stå
 * tom med `finish_reason: "length"` - et tomt svar som ser ut som en modellfeil.
 */
async function callAiFactory(prompt: string, temperature: number, systemMessage: string, signal: AbortSignal, reasoning = false): Promise<Modellsvar> {
  if (!aiFactoryApiKey) {
    throw new Error("TELENOR_AI_FACTORY_API_KEY mangler");
  }
  const modell = reasoning ? aiFactoryReasoningModel : aiFactoryModel;
  ...
  if (!svar.ok) {
    // 503 herfra er nesten alltid taket foran tjenesten, ikke modellen som nekter.
    throw new Error(svar.status === 503
      ? `Telenor AI Factory svarte med status 503. Kallet brukte trolig mer enn ${aiFactoryTimeoutMs} ms hos ${modell}.`
      : `Telenor AI Factory svarte med status ${svar.status}`);
  }
  const data = (await svar.json()) as { choices?: { message?: { content?: string; reasoning_content?: string } }[] };
  const melding = data?.choices?.[0]?.message;
  return {
    tekst: melding?.content?.trim() || "",
    modell: `telenor-ai-factory:${modell}`,
    reasoning: melding?.reasoning_content?.trim() || undefined
  };
}
...
async function writeTrace(linje: Sporlinje): Promise<void> {
  try {
    await mkdir(stateDir, { recursive: true });
    await appendFile(traceFile, JSON.stringify(linje) + "\n", "utf8");
  } catch (feil) {
    console.error(`Kunne ikke skrive KI-spor: ${feilmelding(feil)}`);
  }
}
...
async function buildProviderStatus() {
  ...
  return {
    ...
    "telenor-ai-factory": {
      ...
      // Ikke valgbar i nedtrekkslisten: den gjelder bare de oppgavene som ber om
      // reasoning, og hvilken modell som klarer dem er målt, ikke en smakssak.
      reasoningModel: aiFactoryReasoningModel,
      ...(reasoningValg.advarsel ? { reasoningAdvarsel: reasoningValg.advarsel } : {}),
...
async function callModel(prompt: string, valg: Modellvalg = {}): Promise<Modellsvar> {
  ...
  try {
    ...
    await writeTrace({
      ...
      model: svar.modell,
      response: svar.tekst,
      // Tenkingen er ikke svaret, men den er det eneste stedet man ser hvorfor
      // svaret ble som det ble. Uten den i sporet er en reasoning-modell mindre
      // etterprøvbar enn en uten, ikke mer.
      reasoningResponse: svar.reasoning ?? null,
...
async function judgeWithAi(body: AiKropp) {
...
async function getProcessChoiceFromModel(body: AiKropp) {
...
async function interpretReplyWithAi(body: AiKropp) {
...
async function chooseProcessWithAi(body: AiKropp) {
  ...
  try {
    ...
    return {
      ...
      syntetisk: true,
      modell: `${aiProvider || "mock"}-fallback`,
      advarsel: `LLM-prosessvalg feilet: ${feilmelding(error)}`
    };
  }
}

async function buildAiResponse(type: string, body: AiKropp) {
 ...
 */
async function answerCitizenQuestion(body: AiKropp) {
  const sprak = body?.sprak || "nb";
  ...
  const avvis = (aarsak: string, advarsel: string) => ({
    ...base,
  ...
  try {
    if (url.pathname === "/helse") {
      ...
      jsonResponse(response, 200, {
        ...
        // En reasoning-oppgave hos en provider uten støtte kjører uten tenking. Det
    ...
    jsonResponse(response, 500, { feil: "Intern feil i AI-gateway.", detalj: feilmelding(error), syntetisk: true });
  }
});
