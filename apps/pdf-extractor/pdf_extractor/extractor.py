...
def build_review(result: ExtractionDocument) -> str:
    ...
    reasons = "".join(f"<li>{html.escape(str(item))}</li>" for item in result.quality.get("qualityReasons", []))
    needs_check = knowledge_status(result) == "check-recommended"
    quality_class = "warning" if needs_check else "ready"
    quality_label = "Kontroll anbefales" if needs_check else "Ingen kjente uttrekksproblemer"
    return f'''<!doctype html>
<html lang="nb"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width">
<title>PDF-uttrekk – {html.escape(result.documentId)}</title>
<style>body{{font:16px system-ui;margin:0;color:#1f2a37}}header{{padding:24px;background:#eef6ff}}section{{display:grid;grid-template-columns:minmax(420px,1fr) 1fr;gap:24px;padding:24px;border-top:1px solid #ccd5df}}img{{width:100%;border:1px solid #9aa8b5}}article{{padding:12px;margin:0 0 12px;border-left:6px solid #777;background:#f7f8fa}}article.binding{{border-color:#9c2c2c}}article.guidance{{border-color:#1769aa}}small{{color:#596674}}.knowledge-links{{display:flex;gap:10px;flex-wrap:wrap;margin:14px 0}}.knowledge-links a{{padding:8px 12px;border:1px solid #66809a;border-radius:7px;background:#fff;color:#174d78;text-decoration:none;font-weight:600}}.quality{{margin-top:20px;padding:16px;border:2px solid #217a3c;border-radius:8px;background:#edf9f0}}.quality.warning{{border-color:#b56b00;background:#fff7e6}}.badge{{display:inline-block;padding:5px 10px;border-radius:999px;background:#aee0b8;font-weight:700}}.warning .badge{{background:#f5cf83}}@media(max-width:900px){{section{{grid-template-columns:1fr}}}}</style></head>
<body><header><h1>{html.escape(str(result.document.get("title")))}</h1>
<p>Profil: {html.escape(result.profile)} · bildeanalysemodell: {model} ({availability}) · skjema {SCHEMA_VERSION}</p>
<nav class="knowledge-links" aria-label="Dokumentversjoner"><a href="/dokumenter/{html.escape(result.documentId)}/kunnskap.md">Kompakt kunnskap</a><a href="/dokumenter/{html.escape(result.documentId)}/biter">RAG-biter</a><a href="/dokumenter/{html.escape(result.documentId)}/uttrekk">Detaljert JSON</a></nav>
<div class="quality {quality_class}"><span class="badge">{quality_label}</span><p>Dokumentet er søkbart. Bruk kildepekere og kvalitetsflagg når svaret må kontrolleres.</p><ul>{reasons or warnings}</ul></div>
