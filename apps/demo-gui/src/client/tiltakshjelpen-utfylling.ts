export type TiltakshjelpenInputField = {
  id: string;
  label: string;
  hint?: string;
  type: "tall" | "valg";
...
export type TiltakshjelpenDialogReply = {
...
type Options = {
  fields: readonly TiltakshjelpenInputField[];
  locked: () => boolean;
  changed: () => void;
  ask: (fieldId: string, tekst: string, signal: AbortSignal) => Promise<TiltakshjelpenDialogReply>;
};

export function createTiltakshjelpenUtfylling(options: Options) {
  ...
  const input = (field: TiltakshjelpenInputField) => el<HTMLInputElement | HTMLSelectElement>(field.id);
  ...
  const groups: TiltakshjelpenInputField[][] = [];
  ...
  const groupIndex = (field: TiltakshjelpenInputField) => groups.findIndex(group => group.includes(field));
  ...
  function placeholder(field: TiltakshjelpenInputField): string {
  ...
  function label(field: TiltakshjelpenInputField): string {
  ...
  function valid(field: TiltakshjelpenInputField, report = true): boolean {
  ...
  function selectField(field: TiltakshjelpenInputField): void {
    rememberDraft();
    clearConversation();
    fieldId = field.id;
  ...
  function confirmField(field: TiltakshjelpenInputField): void {
