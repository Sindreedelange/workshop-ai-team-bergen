...
import { createTiltakshjelpenUtfylling, type TiltakshjelpenDialogReply } from "./tiltakshjelpen-utfylling.ts";
import { projectTiltakshjelpenDialogGrunnlag } from "../../../shared/tiltakshjelpen-dialog.ts";
...
import { createTiltaksvalg } from "./tiltakshjelpen-tiltak.ts";
import { renderTiltaksraad, type Tiltaksraad } from "./tiltakshjelpen-raad.ts";

type TiltakshjelpenSvar = { grunnlag: TiltakshjelpenGrunnlag; vurdering: TiltakshjelpenVurdering; sporingsId: string; raad?: Tiltaksraad };
...
let idportenBase = "";
let agentBase = "";
let utfylling: ReturnType<typeof createTiltakshjelpenUtfylling> | undefined;
...
let valgtAdresse: TiltakshjelpenAdresse | null = null;
let plassering: TiltakshjelpenPunkt | null = null;
let grunnlag: TiltakshjelpenGrunnlag | null = null;
let kartgrunnlag: TiltakshjelpenGrunnlag | null = null;
...
let planflater: TiltakshjelpenPlanflate[] = [];
let plankilde: TiltakshjelpenGrunnlag["kilder"][number] | undefined;
let vurdering: TiltakshjelpenSvar | null = null;
...
const form = krevEl<HTMLFormElement>("measure-form");
...
function renderPropertySteps(): void {
  ...
  krevEl("measure-step").hidden = !placementConfirmed || !tiltakstypeBekreftet;
...
async function searchAdresse(tekst: string, kommunenummer?: string): Promise<void> {
  ...
  const treff = await api<TiltakshjelpenAdresse[]>(`/api/garasje/adresser?${search}`);
  if (!treff.length) {
...
async function selectAdresse(adresse: TiltakshjelpenAdresse): Promise<void> {
  clearConfirmation();
...
function formatEiendomsdetaljer(adresse: TiltakshjelpenAdresse): string {
...
function renderGrunnlag(data: TiltakshjelpenGrunnlag): void {
...
function xy(punkt: TiltakshjelpenPunkt): [number, number] {
  return projectTiltakshjelpenPunkt(punkt, bounds);
}
...
function renderBuildings(data: TiltakshjelpenGrunnlag): void {
...
function renderNeighbours(data: TiltakshjelpenGrunnlag): void {
...
function renderMap(data: TiltakshjelpenGrunnlag): void {
  kartgrunnlag = data;
...
function moveMarker(punkt: TiltakshjelpenPunkt): void {
...
function renderFields(): void {
  const container = krevEl("measure-fields");
...
function renderVurdering(data: TiltakshjelpenSvar): void {
  vurdering = data;
  ...
  const svaret = beskrivTiltakshjelpenUtfall(data.vurdering);
  ...
  const fulfilled = krevEl("fulfilled-checks");
  list.replaceChildren();
  fulfilled.replaceChildren();
  const status = { oppfylt: "Oppfylt", brudd: "Ikke oppfylt", uavklart: "Må avklares" };
  const priority = { brudd: 0, uavklart: 1, oppfylt: 2 };
  for (const sjekk of [...data.vurdering.sjekker].sort((a, b) => priority[a.status] - priority[b.status])) {
    const li = element("li");
    const heading = element("h4", `${status[sjekk.status]}: ${sjekk.navn}`, "ds-heading");
    heading.dataset.size = "xs";
    li.append(heading, element("p", sjekk.forklaring, "ds-paragraph"));
    addLink(li, "Se kilde", sjekk.kilde);
    (sjekk.status === "oppfylt" ? fulfilled : list).append(li);
  }
  const caveats = krevEl("result-caveats");
  caveats.replaceChildren();
  const shown = new Set(data.vurdering.sjekker.map(sjekk => sjekk.forklaring));
  for (const forhold of new Set(data.vurdering.uavklarteForhold)) {
    if (!shown.has(forhold)) caveats.append(element("li", forhold));
  }
  krevEl("result-follow-up").hidden = list.children.length === 0;
  krevEl("result-limitations").hidden = caveats.children.length === 0;
  krevEl("result-fulfilled").hidden = fulfilled.children.length === 0;
...
async function fetchTiltaksraad(data: TiltakshjelpenSvar): Promise<Tiltaksraad> {
  const projected = projectTiltakshjelpenDialogGrunnlag(data.grunnlag);
...
function configureFields(type: Byggetiltakstype): void {
  ...
  utfylling = createTiltakshjelpenUtfylling({
  fields,
  ...
  ask: async (feltId, tekst, signal) => {
    ...
    const response = await fetch(`${agentBase}/agent/garasje/dialog`, {
      ...
      body: JSON.stringify({
        ...
        kontekst: {
          prosjekt: { tiltakstype },
          resultater: projectTiltakshjelpenDialogGrunnlag(grunnlag),
          samtale: helpHistory.slice(-4).map(turn => ({ rolle: turn.rolle, tekst: turn.tekst.slice(0, 1000) }))
        }
      })
    });
    const data: TiltakshjelpenDialogReply & { feil?: string } = await response.json();
...
function placeFromPointer(event: PointerEvent): void {
  ...
  if (!matrix) {
    krevEl("placement-note").textContent = "Kartet er ikke klart. Prøv igjen når kartet vises.";
    return;
  }
  const point = new DOMPoint(event.clientX, event.clientY).matrixTransform(matrix.inverse());
  moveMarker(unprojectTiltakshjelpenPunkt(point.x / 640, point.y / 480, bounds));
}
let activeMapPointer: number | null = null;
svg.addEventListener("pointerdown", event => {
  if (busy || pendingSave || !propertyConfirmed || placementConfirmed || !valgtAdresse
    || !event.isPrimary || event.button !== 0 || activeMapPointer !== null) return;
  event.preventDefault();
...
svg.addEventListener("pointermove", event => {
  if (event.pointerId !== activeMapPointer) return;
  event.preventDefault();
  placeFromPointer(event);
});
...
form.addEventListener("submit", event => {
  ...
  void perform("Kontrollerer opplysninger og henter ferskt plangrunnlag …", async () => {
    ...
    const data = await api<TiltakshjelpenSvar>(`/api/garasje/sjekk?${query}`);
...
void perform("Klargjør tiltakssjekken …", async () => {
  ...
  if (embedded) {
    ...
    if (pageParams.get("integrert") === "resultat") {
      ...
      const result = data as TiltakshjelpenSvar;
      tiltaksvalg?.hide();
