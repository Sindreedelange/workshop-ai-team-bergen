import type { TiltakshjelpenPolygon, TiltakshjelpenPunkt } from "../../../shared/tiltakshjelpen.ts";
...
export type TiltakshjelpenGrenseavstand = {
...
export function nearestPolygonBoundary(punkt: TiltakshjelpenPunkt, polygons: readonly TiltakshjelpenPolygon[]): TiltakshjelpenGrenseavstand | null {
...
export function fitKartutsnitt(polygons: TiltakshjelpenPolygon[], center: TiltakshjelpenPunkt): Kartutsnitt {
...
export function projectTiltakshjelpenPunkt(punkt: TiltakshjelpenPunkt, bounds: Kartutsnitt): [number, number] {
  return [
...
export function unprojectTiltakshjelpenPunkt(x: number, y: number, bounds: Kartutsnitt): TiltakshjelpenPunkt {
  return {
...
export function findNabotomtLabel(
  polygon: TiltakshjelpenPolygon, selected: TiltakshjelpenPolygon[], bounds: Kartutsnitt, width: number, occupied: KartLabel[]
): KartLabel | null {
  const rings = polygon.ringer.map(ring => ring.map(([lon, lat]) => projectTiltakshjelpenPunkt({ lon, lat }, bounds)));
  const excluded = selected.map(shape => shape.ringer.map(ring => ring.map(([lon, lat]) => projectTiltakshjelpenPunkt({ lon, lat }, bounds))));
